...
public class SimpleType implements Type {
	...
	static {
		for (Class<?> cl : Arrays.<Class<?>>asList(byte.class, int.class, long.class, short.class, float.class,
	...
	public SimpleType(String simpleName) {
		this(TypeCategory.SIMPLE, simpleName, StringUtils.EMPTY, simpleName, false, false);
	}

	public SimpleType(Type type, List<Type> parameters) {
		this(type.getCategory(), type.getFullName(), type.getPackageName(), type.getSimpleName(), type.isPrimitive(),
				type.isFinal(), parameters);
	}

	public SimpleType(Type type, Type... parameters) {
		this(type.getCategory(), type.getFullName(), type.getPackageName(), type.getSimpleName(), type.isPrimitive(),
				type.isFinal(), Arrays.asList(parameters));
	}
	...
	public SimpleType(TypeCategory category, String fullName, String packageName, String simpleName,
		...
		if (localName.contains(DOT)) {
			this.outerClassName = fullName.substring(0, fullName.lastIndexOf(DOT));
		} else {
		...
		for (Type type : parameters) {
			/** 如果泛型引用是自身 */
			if (type instanceof TypeThis) {
				((TypeThis) type).setType(this);
			}
		}

		this.parameters = parameters;
		this.memberClass = localName.contains(DOT);
	}

	public SimpleType(TypeCategory typeCategory, String fullName, String packageName, String simpleName, boolean p,
			boolean f, Type... parameters) {
	...
	public Type asArrayType() {
		if (arrayType == null) {
			String newFullName = getFullName() + ARRAY;
			String newSimpleName = getSimpleName() + ARRAY;
	...
	public Type getComponentType() {
		if (fullName.endsWith(ARRAY)) {
			if (componentType == null) {
				...
				componentType = new SimpleType(TypeCategory.SIMPLE, newFullName, getPackageName(), newSimpleName, false,
						false);
			}
	...
	public Type getEnclosingType() {
		if (enclosingType == null && memberClass) {
			String newLocalName = localName.substring(0, localName.lastIndexOf(DOT));
			String newSimpleName = newLocalName.substring(newLocalName.lastIndexOf(DOT) + 1);
	...
	public Class<?> getJavaClass() {
		if (javaClass == null) {
			String className;
			if (packageName.length() > 0) {
				className = packageName + DOT + localName.replace(DOT, DOLLAR);
			} else {
				className = localName.replace(DOT, DOLLAR);
			}
			try {
	...
	public String getRawName(Set<String> packages, Set<String> classes) {
		if (classes.contains(fullName)) {
			...
			return fullName.substring(outerClassName.lastIndexOf(DOT) + 1);
